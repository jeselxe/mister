defmodule Mister.Report do
  @moduledoc """
  Construcción **pura** del informe diario.

  `build/1` convierte los datos ya calculados (presupuesto, candidatos,
  valoraciones, clausulazos, alineación) en el mapa del informe, cruzando las
  fuentes:
  ...
  `actions/1` deriva el checklist de acciones del informe. La persistencia vive
  aparte, en `Mister.Reports`.
  """

  import Mister.Format, only: [money: 1]

  alias Mister.Report.Input
  alias Mister.{SaleEstimator, Valuation}
  ...
  @doc "Construye el mapa del informe diario a partir de `Mister.Report.Input`."
  def build(%Input{} = input) do
    %Input{
      budget: budget,
  ...
  @doc """
  Deriva el checklist de acciones (atributos listos para `report_actions`) del
  mapa del informe. Puro: `Mister.Reports.persist!/1` lo persiste.
  """
  def actions(report_map) do
    clause_actions =
